...
<script type="text/javascript">
	$(function(){
		$("#oldpassword").blur(function(){
			var pwd = $(this).val();
			var id = $("#userid").val();
			$.post("${pageContext.request.contextPath }/user/user.do",{"pwd":pwd,"id":id,"method":"isPwd"},function(result){
				if(result.flag==0){
	...
	function save() {
			...
			if (newpwd == rnewpwd) {
				$.post("${pageContext.request.contextPath }/user/user.do",{"userpassword":userpassword,"id":id,"method":methods},function(result){
					if(result > 0){
						alert("密码修改成功，请重新登录！");
						window.location.href="http://localhost:8080/smbms/login.jsp";
...
<div class="right">
            ...
            <div class="providerAdd">
                <form id="userForm" name="userForm" method="post" action="">
                    <input type="hidden" id="userid" value="${userSession.id}">
                    <input type="hidden" id="methods" value="savePwd">
